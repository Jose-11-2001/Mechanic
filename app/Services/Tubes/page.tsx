...
export default function Tubes() {
  ...
  const category = 'tubes'; // Fixed category for this page
        ...
        {editingItem && (
          <div className="bg-gray-800 rounded-2xl p-6 mb-6 border border-blue-500">
            <h3 className="text-xl font-bold text-white mb-4">
              {editingItem.id > 1000 ? 'Add New Tube' : 'Edit Tube'}
            </h3>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {renderField('Size', editingItem.size, 'size')}
              {renderField('Type', editingItem.type, 'type')}
              {renderField('Valve Type', editingItem.valve, 'valve')}
            ...
            <div className="flex space-x-2 mt-4">
              <button
                ...
                className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg"
              >
                {editingItem.id > 1000 ? 'Add Tube' : 'Update Tube'}
              </button>
              <button
              ...
              <div className="flex justify-between items-center">
                <div className="flex-1">
                  ...
                  <p className="text-gray-300">
                    ...
                    {item.valve && `Valve: ${item.valve} • `}
                    Price: MK {item.price?.toLocaleString('en-MW')} • Qty: {item.quantity}
                  </p>
                </div>
